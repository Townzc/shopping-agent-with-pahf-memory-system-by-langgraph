...
| 测试文件 | 覆盖能力 | 结果 |
| --- | --- | --- |
| `test_api.py` | 健康检查、聊天 API、参数校验、stream 占位接口 | 通过 |
...
| 接口 | 操作 | 实际结果 | 结论 |
| --- | --- | --- | --- |
| `/api/v1/shop/categories` | GET | 返回 `家居`、`数码3C`、`服饰`、`美妆个护` | 通过 |
...
| 测试项 | 未完成原因 | 补测建议 |
| --- | --- | --- |
| 真实模型普通自动回复 | 无真实 API_KEY，本轮只配置 `sk-local-test` | 准备模型服务密钥，跑售后问答集 |
